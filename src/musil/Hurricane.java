...
public class Hurricane {
    ...
    static String name;
    static String month;
    static Hurricane[] hrs; //JV tato trida by mela byt zodpovedna za jeden hurikan
    ...
    public static void addHurricane(File file) {
        try {
            ...
            while ((data = br.readLine()) != null) {
                ...
                String[] parts = data.split(" +"); //Dejme tomu že by tohle fungovalo, teďka netuším jak to má být konkrétně správně
                year = parts[0].; //Nutno přeparsovat
                pressure = parts[2].;
                knots = parts[3].;
                name = parts[4];
                month = parts[1];
                Hurricane hr = new Hurricane(int year
                , String month, int pressure, int knots, String name
            
            );
        }
        br.close(); //JV lepsie pouzit try with resources, tento close() sa nezavola, ked nastane vynimka a to nie je dobre
        } catch (IOException e) {
            System.out.println("Problém se souborem" + e.getMessage()); //JV ak ked sa 
        }
    }
    ...
    public static void xToY() {
        ...
        for (Hurricane r : hrs) {
            ...
            if ((actualYear <= yearY) && (actualYear >= yearX)) {
                System.out.println(r.toString()); //JV okrem UI by nikde nemal byt sout
            }
        }
    }
    ...
    public static void infoByName() {
        System.out.println("zadejte prosím jméno daného hurikánu"); //JV okrem UI by nekde nemal byt Scanner nacitania z konzole
        String hName = "";
        String currentHName;
        int end = 0;
        while (true) {
